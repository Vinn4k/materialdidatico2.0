...
class PdfViewerControllerUi extends GetxController {
  ...
  RxBool pdfDownloadComplete = true.obs;

  Future<DocumentSnapshot> getUserInfo() async {
    User? user = FirebaseAuth.instance.currentUser;
    String uid = user?.uid ?? "";
    DocumentSnapshot snapshot = await _repository.getUserInfo(uid);
  ...
  Future<void> downloadPdf(
    ...
    pdfDownloadComplete.value = false;
    await _service
        .downloadPdf(url: url, fileName: fileName)
        .then((value) => pdfDownloadComplete.value = true)
        .onError((error, stackTrace) => pdfDownloadComplete.value = true);
  }

  String pdfDec(String path) {
    return _service.decryptPdf(pdfPath: path);
  }

  Future<String> pdfIsSync() async {
    String id = data['id'] ?? "ERROR";
    final isOffline = await _service.pdfIsSync(id: id);
    if (isOffline != "nd") {
      offPdfBase64.value = isOffline;
    }
